...
\renewcommand{\thepage}{\Roman{page}}
	...
	在模型方面，本文选取一维立方振子和随机 Canard 慢–快系统作为原型。时间离散侧采用驯化 Euler 与截断 Euler–Maruyama；空间离散侧采用 CTRW 格式 \(Q_u\) 及其改进格式 \(\widetilde Q_u\)。对一维立方振子，本文在漂移主导区域引入固定空间跨越距离的时间尺度，将确定性漂移时间、CTRW 的平均驻留时间以及真实 SDE 的平均首达时间（MFPT）置于同一渐近框架，并给出 MFPT 的局部渐近展开，从而定量比较时间离散与空间离散在 MFPT 上的误差阶和适用区间。
	
	对于随机 Canard 系统，本文引入 committor 函数和 MFPT 作为长期弱量指标，从生成元视角得到其满足的椭圆型边值问题，并构造相应的离散线性方程 \(Q q_h=0,\ Q m_h=-1\)。在合理的光滑性和一致逼近假设下，可以证明改进格式 \(\widetilde Q_u\) 对 committor 与 MFPT 具有高阶空间收敛；相应地，基于驯化与截断 EM 的 Monte Carlo 估计，其弱收敛阶通常不超过 \(O(\Delta^{1/2})\)，为达到同等精度需要显著更高的计算代价。
	
	数值实验方面，本文通过路径几何、首达时间统计以及 committor/MFPT 的热力图，对比两类方法在慢–快结构、转迁概率与长期统计性质上的表现，并在统一的误差–工作量（work–error）度量下给出定量曲线。结果表明：空间离散方法在弱量和长期统计方面明显更高效，而时间离散方法在捕捉单条轨道几何和短时动力学方面更具灵活性。本文的分析和实验为今后在复杂随机动力系统中选择和设计合适的数值格式提供了参考。
	\\   % 摘要文本与关键词之间空一行
...
Stochastic differential equations (SDEs) arise in molecular dynamics, mathematical finance and biological systems. For SDEs with locally Lipschitz drift of superlinear growth, classical time–discretization schemes such as Euler–Maruyama may diverge, whereas generator–based space discretizations, for example continuous–time random walks (CTRW), often offer better stability and robustness for long–time statistics. This thesis compares time and space discretizations within a unified framework, focusing on their accuracy, efficiency and suitability for describing long–time behaviour.

Two prototype models are considered: a one–dimensional cubic oscillator and a stochastic Canard slow–fast system. On the time–discretization side we use the tamed Euler and truncated Euler–Maruyama schemes; on the space–discretization side we use the CTRW scheme \(Q_u\) and its improved version \(\widetilde Q_u\). For the cubic oscillator, in the drift–dominated regime we introduce a time scale based on a fixed spatial crossing length and compare the deterministic drift time, the mean holding time of the CTRW process and the true mean first passage time (MFPT) of the SDE. Local asymptotic expansions of the MFPT quantify the error orders of time and space discretizations and clarify their respective regimes of validity.

For the stochastic Canard system, we take the committor function and MFPT as long–time weak observables and derive their elliptic boundary value problems from the infinitesimal generator, leading to discrete linear systems \(Q q_h=0\) and \(Q m_h=-1\). Under standard smoothness and consistency assumptions, the improved scheme \(\widetilde Q_u\) achieves higher–order spatial convergence for both committor and MFPT. In contrast, Monte Carlo estimators based on tamed and truncated Euler schemes typically have weak convergence orders not exceeding \(O(\Delta^{1/2})\), which implies a higher computational cost for the same accuracy.

Numerical experiments compare the two classes of schemes in terms of trajectory geometry, first–passage–time statistics and spatial structures of committor and MFPT, visualized via heatmaps and slices along the slow manifold. Work–error curves based on root–mean–square errors versus computational effort show that space discretizations are more efficient for weak and long–time quantities, while time discretizations remain advantageous for resolving individual trajectories and short–time dynamics. These results provide practical guidance for choosing numerical schemes in more complex stochastic dynamical systems.
	\\   % 摘要文本与关键词之间空一行
	
	{\bf\large Key words:}  stochastic differential equations; time discretization; space discretization; continuous–time random walk; tamed Euler; truncated Euler–Maruyama; committor function; 
	%%=====================================================================================================%%
	%%
...
%		\item[$Q_c$] Eric 提出的有限体积格式
		\item[$\widetilde{Q}_u$] Zu 提出的改进的有限差分格式
...
%		\item[$t^c$] $Q_c$ 格式对应的平均等待时间 (mean holding time)
	...
	对于时间离散方法， 1999 年学者 Kloeden 和 Platen\cite{kloeden1992stochastic}系统描述了具有不同收敛阶的求解随机微分方程的数值方法。1955年，Maruyama \cite{Maruyama1955}首次提出了 Euler- Maruyama 方法。Euler-Maruyama 方法是时间离散近似最常用的方法，其数值方法如下：
	\begin{equation}
	...
	\begin{equation}
	X_{k+1}=X_{k}+\mu \left(X_{k}\right) h+\sigma \left(X_{k}\right)\left(W_{k+1}-W_{k}\right)+\frac{1}{2} \sigma\left(X_{k}\right) \sigma^{\prime}\left(X_{k}\right)\left[\left(W_{k+1}-W_{k}\right)^{2}-h\right]
	\end{equation}
	...
	\end{equation}
	该方法在Euler–Maruyama方案的基础上对漂移项进行了驯服处理：在每个时间步，使用有界函数削弱漂移的大小，从而将数值增量中的漂移量限制在一个上界以内。这种处理避免了漂移过大导致的单步跳跃失控，从而抑制了解的发散。值得注意的是，Tamed Euler方法仍保持与EM相同的强收敛阶（$1/2$阶），适用于具有超线性增长和全局单侧 Lipschitz 连续漂移系数和全局Lipschitz的扩散系数情形。
	...
	\end{equation}
	其中$\sigma=\operatorname{diag}\left(\sigma_{11}, \sigma_{22}, \ldots, \sigma_{n n}\right)$ 是噪声强度，并且对于 $i=1,2, \ldots, n, \sigma_{i i} \in \mathbb{R}^{+}$。当对方程\eqref{add noise equation}应用连续时间随机游走方法$Q_{u}$时漂移项会产生额外的扩散项。针对这一不足，祖建\cite{zu2023random}提出了一种改进的$Q_u$跳跃格式。该方法在原有$Q_u$方案基础上，通过减小式\eqref{add noise equation}中的 $\sigma$ 来补偿这种额外的扩散项，等价于对跳跃过程附加了校正的扩散项。理论分析表明，$\tilde{Q}_u$格式相较原始的$Q_u$和$Q_c$方案在精度上表现更好，特别是在小噪声情形下依然适用。进一步地，$\tilde{Q}_u$方案在平均驻留时间的渐近展开上具有良好的性质，可保证当网格加密时数值平均步长逼近解析值并达到高阶收敛。
	...
	本文围绕随机动力系统的数值模拟，分别从时间尺度与空间尺度两个角度，对两类离散格式的性能进行系统比较。具体而言，一方面，我们通过考察系统在跨越固定空间距离 \(L\) 时所需的时间，并与连续模型的真实时间进行对比，用以评估两种格式在时间尺度上的逼近能力；另一方面，我们通过分析典型慢–快随机系统的相空间动力学行为，借助先达概率函数（committor function）作为弱量指标，比较两类格式在长期行为和转迁结构上的刻画效果。
	
	在时间尺度的比较方面，本文首先选取一维立方振子模型作为原型问题。在该模型中，当 \(X_t\) 取大值时，方程~\eqref{add noise equation} 的演化主要由漂移项主导，此时可用
	\begin{equation}\label{drift dominated}
		d X_{t}=\mu\left(X_{t}\right)\, d t
	\end{equation}
	给出的漂移主导常微分方程来近似随机动力学。由此得到的跨越距离 \(L\) 所需时间记为 \(t^{e}\)，其主导部分记为 \(t^{*}\)。对于基于生成元空间离散的连续时间随机游走（CTRW）方法，我们将从 \(x\) 运动到 \(x+L\) 的平均等待时间记为 \(t^{u}\)，并将 \(t^{u}\) 与 \(t^{e}\) 进行比较。
	
	对于漂移和扩散项仅满足局部 Lipschitz 且可能超线性增长的随机微分方程，经典的 Euler--Maruyama 格式在强意义和弱意义下都可能失效或发散。为此，本文采用截断 Euler--Maruyama（truncated EM）格式和驯化 Euler（tamed EM）格式作为改进的时间步进方法。记在这两种时间离散方法下，从 \(x\) 运动到 \(x+L\) 的平均首达时间分别为 \(t^{\Delta}\) 和 \(t^{\delta}\)，并将它们与漂移主导时间 \(t^{e}\) 进行对比。该评价准则可理解为：一方面比较空间离散方法中由生成元 \(Q\) 所诱导的平均停留时间 \(t^{u}\)，另一方面比较时间离散方法中由等效时间步长产生的平均首达时间 \(t^{\Delta}, t^{\delta}\)，从而在相同空间跨越尺度下直观比较两类算法的时间刻画能力与数值偏差。
	
	在动力学行为的比较方面，本文选取具有典型慢–快结构的随机 Canard 系统作为测试模型。该系统在相空间中呈现出沿慢流形缓慢演化、随后发生快速跃迁（fast jump）的典型 Canard 现象。为了评估两类格式对真实动力学行为的刻画能力，本文引入先达概率函数（committor function）作为弱量指标：通过构造对应于两个不同慢流形区域的集合 \(A,B\)，比较空间离散与时间离散格式所得到的 committor 函数与参考解之间的偏差，从定性结构（相空间图像）和定量误差（work--error 曲线）两个层面评价两类方法在长期行为描述上的优劣。
	
	本文结构安排如下：第一章介绍课题背景和研究动机；第二章总结随机微分方程数值解的理论基础，包括解的存在唯一性、基本数值方法及其收敛性理论；第三章针对一维立方振子模型，从理论上分析固定空间跨越距离下的运行时间比较准则，并给出相应的数值实验；第四章以随机 Canard 慢–快系统为例，对比分析空间离散与时间离散方法在动力学行为和 committor 函数上的表现，并通过数值实验进行验证；第五章对全文工作进行总结，并对进一步的研究方向进行展望。
...
%	 2．$\left\{X_{t}\right\}_{t \geq 0}$ 有唯一的平稳分布 $\nu$ ，且对任意的 $x \in \mathcal{X}, P^{t}(x, \cdot)$ 收敛到 $\nu$ ，其中 $\nu$关于 Lebesgue 测度是绝对连续的。
	 ...
	 \end{equation}
	 其中 $q: \Omega \times \Omega \rightarrow[0, \infty)$ 是反应速率函数。假设 $Q$ 是无穷小生成元 $L$ 的好的近似，那么基于随机模拟算法的连续时间随机游走方法描述如下。给定当前状态 $X(t)=x$.\\
	 第一步：通过生成参数为
	 ...
	 以 $n$ 维随机微分方程\eqref{eq:SDE}为例，Bou-Rabee 和 Vanden-Eijnden提出了有限差分离散化 $Q_{u}$ 方法，而且具有一阶精度。 设 $\left\{e_{i}\right\}_{i=1}^{n}$ 为 $\mathbb{R}^{n}$ 上的标准基，有 $n$ 个反应通道方向。在每个反应通道方向上，允许向前／后跳。因此反应通道的总数为 $2 n$ 。设 $h_{i}^{ \pm}(x), h_{i}(x)=\frac{h_{i}^{+}(x)+h_{i}^{-}(x)}{2}$ 为在状态 $x$ 处的向前／后以及平均空间步长。设 $\left\{\sigma_{i}(x)=\sigma e_{i}\right\}_{i=1}^{n}$ 是噪声矩阵 $\sigma(x)$ 的列，且 $\sigma_{i}(x)$ 为第 $i$ 个通道方向。记$\tilde{\mu}(x)$ 为变换后的漂移场，逐点定义为 $M(x) \tilde{\mu}(x)=\mu(x)$,其中 $M(x)=0.5 * \sigma(x) \sigma(x)^{T}$ 是 $n \times n$ 扩散矩阵，则有	
	 ...
	 而对于在 $\mathbb{R}^{n}$ 上边长为 $h$ 的 Cartesian 网格上考虑具有加性噪声的随机微分方程
	 \begin{equation*}
	 d X_{t}=\mu\left(X_{t}\right) d t+\sigma d W_{t} 
	 \end{equation*}
	 其中 $\sigma=\operatorname{diag}\left(\sigma_{11}, \sigma_{22}, \ldots, \sigma_{n n}\right)$ 是噪声强度，并且对于 $i=1,2, \ldots, n, \sigma_{i i} \in \mathbb{R}^{+}$，Li 和 Zu 提出的改进的有限差分离散化 $\tilde{Q_{u}}$ 表示如下：
	
	 \begin{align}
	 \widetilde{Q}_{u} f(x)= & \sum_{i=1}^{n}\left(\frac{\mu_{i}(x) \vee 0}{h}+\frac{M_{i i}}{h^{2}}\right)\left(f\left(x+h e_{i}\right)-f(x)\right)  \\
	 ...
	 & \tilde{Q}_u \text { 步骤 }: M_{i i} \leftarrow 0.5 *\left(\sigma_{i i}^{2}-\left|\mu_{i}(X)\right| h\right) \vee 0   \\
	 & \quad q_{i 1} \leftarrow\left(\mu_{i}(X) \vee 0\right) / h+M_{i i} / h^{2}, q_{i 2} \leftarrow\left(-\mu_{i}(X) \wedge 0\right) / h+M_{i i} / h^{2} 
	 \end{align}	
...
%	其中 $\mu:\mathbb{R}^n\to\mathbb{R}^n$ 为漂移项，$\sigma:\mathbb{R}^n\to\mathbb{R}^{n\times m}$ 为扩散项，$W_t$ 为 $m$ 维 Brownian 运动。
	...
	经典 Euler--Maruyama（EM）格式为
	\[
	X_{k+1} = X_k + \mu(X_k)\,h + \sigma(X_k)\,\Delta W_k.
	\]
	在 $\mu,\sigma$ 全局 Lipschitz 且线性增长的条件下，EM 具有 $1/2$ 阶强收敛及 1 阶弱收敛。然而，当漂移仅局部 Lipschitz、并在无穷远处呈超线性增长时，即便原方程 \eqref{eq:SDE} 本身是良态并具有唯一强解，EM格式也可能在有限时间内数值解的矩发散而失去强收敛性。Hutzenthaler 等人的工作表明，在这类非全局 Lipschitz 情形下，EM 甚至可能在有限时间点 $T$ 处满足
	\[
	\lim_{N\to\infty}
	\Bigl(\mathbb{E}\bigl|X_T - X^{\mathrm{EM}}_N\bigr|^2\Bigr)^{1/2} = +\infty,
	\]
	从而不能作为可靠的强逼近方法。为此，人们提出了对 EM 进行适当修正的改进，其中具有代表性的是驯化 Euler（Tamed Euler）方法 \cite{hutzenthaler2012strong} 和截断 Euler--Maruyama（Truncated EM）方法 \cite{mao2015truncated}。下面分别对这两种格式作简要介绍。
	
	\subsubsection{驯化 Euler 方法（Tamed Euler）}
	...
	在上述假设下，为克服 EM 在大漂移区域由于过大的单步更新而导致的数值发散，\cite{hutzenthaler2012strong} 将漂移增量进行驯化（taming），给出如下数值格式：令
	\[
	t_k = k\delta,\qquad \delta = T/N,
	\]
	驯化 Euler 近似 $\{X_k^{\delta}\}_{k=0}^N$ 定义为
	...
	\begin{theorem}[Tamed Euler 的强收敛性，见 \cite{hutzenthaler2012strong}]
		...
		\begin{equation}\label{eq:tamed-strong}
			...
			\le C_p\,\delta^{1/2}.
		\end{equation}
		也就是说，驯化 Euler 格式在上述非全局 Lipschitz 情形下仍保持 $1/2$ 阶强收敛，与标准 EM 在全局 Lipschitz 情形下的收敛阶一致。
	\end{theorem}
	
	与隐式 Euler 相比，\eqref{scheme:tamed} 在每一步不需要求解非线性方程，计算上与显式 EM 基本同价，因此在大规模 Monte Carlo 模拟中具有明显的效率优势。
	
	\subsubsection{截断 Euler--Maruyama 方法（Truncated EM）}
	
	Hutzenthaler 等人的结果说明：在漂移仅局部 Lipschitz 且超线性增长时，经典 EM 的矩可能发散。Mao 在 \cite{mao2015truncated} 中进一步考虑更一般的局部 Lipschitz 情形，并假设漂移与扩散满足 Khasminskii 型条件：存在常数 $K>0$、$p>2$，使得
	\begin{equation}\label{Khasminskii-again}
		x^{\top}\mu(x)+\frac{p-1}{2}\|\sigma(x)\|^2
		\le K\bigl(1+|x|^2\bigr),
		\qquad \forall x\in\mathbb{R}^n.
	\end{equation}
	在 \eqref{Khasminskii-again} 条件下，方程 \eqref{eq:SDE} 仍存在唯一强解，并具有所有阶矩有界。然而，直接应用 EM 仍可能发散。为此，\cite{mao2015truncated} 先对漂移和扩散在空间上做截断，使其在 $\mathbb{R}^n$ 上有界，再在截断系数上使用标准 EM。
	...
	\begin{theorem}[Truncated EM 的强收敛性，见 \cite{mao2015truncated}]
		...
		\begin{equation}\label{eq:trunc-strong}
			...
			\le C_q\,\Delta^{1/2}.
		\end{equation}
		也就是说，在局部 Lipschitz $+$ Khasminskii 条件下，截断 EM 格式同样达到 $1/2$ 阶强收敛。
	\end{theorem}
	
	与驯化 Euler 不同，截断 EM 并不直接修改漂移增量的形式，而是通过投影 $\Pi_\Delta$ 将状态限制在一个随步长 $\Delta$ 放大的有界区域内，再在该区域上应用标准 EM。这样一来，离散系数 $\mu_\Delta,\sigma_\Delta$ 全局有界，从而避免了数值解的爆炸；同时，由于 $R_\Delta\to\infty$，截断误差在 $\Delta\to0$ 时可以被严格控制。
	
	\medskip
	
	综上所述，驯化 Euler 与截断 EM 都是在保持方法显式性的前提下，对漂移/扩散项进行适度修正，以恢复在非全局 Lipschitz 情形下的强收敛性。前者通过对漂移增量施加非线性缩放，后者则通过空间截断保证系数有界。本文后续章节将以 \eqref{scheme:tamed} 与 \eqref{scheme:trunc} 为时间离散的两种代表性的改进 EM 格式，并在一维立方振子与随机 Canard 系统等模型上，从强收敛、长时间行为以及 committor/MFPT 等弱指标的角度，系统比较它们与空间离散方法的数值表现。
...
\end{equation}
其中$x\gg 1$，目标阈值 $L=x-\delta$，且
\begin{equation}\label{eq:assum-local}
	0<\delta\ll x,\quad \mu(x)<0,\quad \text{并在 }[L,x]\text{ 上 }\mu\ \text{光滑}.
\end{equation}
...
由\eqref{t^u} 可见，当 $|x_i|$ 很大时，$t^u$ 会趋近于 $t^{\ast}$，\cite{bou2018continuous}中仅给出了\(t^u\)与近似平均等待时间\(t^*\)的比较，本文给出\(t^u\)与精确平均等待时间\(t^e\)的比较
\begin{theorem}
...
\end{theorem}
%
%同样地，如果 (3.3.5) 中的第二项相对第一项衰减得更快，那么 $t^{e}$ 与 $t^{\ast}$ 的相对误差也趋于 0；因此，$Q_u$ 给出的平均停留时间估计在渐近意义下与精确平均停留时间一致。举例来说，若 $\mu(x)$ 的主导项形如 $-a x^{2p+1}$（其中 $p\ge 0,\ a>0$），则该第二项衰减得足够快。
%
...
%
%
%下面给出两节可直接并入你正文的 LaTeX 文稿（不含导言与 \verb|\documentclass| 等），完全围绕你设定的顺漂移 + 小位移（(x\gg1,\ L=x-\delta,\ \delta\ll x)）场景来展开。第一节分析**时间离散格式**下的平均首达时间（MFPT），第二节给出与你已在第三章中建立的**空间离散平均驻留时间**的渐近误差对比：(|\text{Mean Holding Time} - t^e|) 与 (|\text{MFPT} - t^e|)。为与前文符号一致，我沿用你在符号和缩略语说明中给出的 (t^e,t^*,t^u,t^c,\tilde t^u,t^{\Delta},t^{\delta}) 等记号，并明确指出适用的渐近假设与误差主项。

%===============================

\subsection{时间离散格式平均首达时间（MFPT）的推导与局部渐近}\label{subsec:mfpt-pre-asym}
...
\end{equation}
对 \eqref{eq:mfpt-def-local} 的 $\tau$，由生存概率法\cite{weinan2021applied}或 Dynkin 公式可得后向方程
...
%	\)；
%	在本文的顺漂移 + 小位移数值设定下（二者均远小于主导项），结论不受影响。
%	%===============================


%\subsection{时间离散格式的平均首达时间分析} %（顺漂移 + 小位移极限）
...
\subsection{时间离散格式的平均首达时间：渐近一致性与误差估计}

为便于比较，记
\begin{equation}\label{eq:te-def}
	t^{e}(x,\delta)\;:=\;\int_{x-\delta}^{x}\frac{dy}{\lvert \mu(y)\rvert},
	\qquad x\gg1,\quad 0<\delta\ll x,
\end{equation}
为在“顺漂移+\ 小位移”极限下的\emph{漂移主导}解析时间（cf.\,式~\eqref{eq:te-def} 与前文的漂移主导讨论）。记 Tamed--EM 的一步更新为
\[
X_{n+1}
=
X_n
...
\(t^{\delta}(x,\delta;\Delta)\) 与 \(t^{\Delta}(x,\delta;\Delta)\)。

\begin{theorem}[Tamed--EM 的 MFPT 渐近与误差]\label{thm:tamed-mfpt}
	设 \(\mu\in C^{2}\) 且在 \(x\) 的邻域内单调并满足 \(\mu(x)<0\)（向左顺漂移），\(\sigma\) 有界。
	在极限 \(x\to\infty,\ \delta/x\to0,\ \Delta\to0\) 下，有
	\begin{equation}\label{eq:tamed-expansion}
		t^{\delta}(x,\delta;\Delta)
		\;=\;
	...
	\begin{equation}\label{eq:tamed-vs-te}
		t^{\delta}(x,\delta;\Delta)-t^{e}(x,\delta)
		\;=\;
		\delta\,\Delta
		\;-\;\frac{\mu'(x)}{2\,\mu(x)^{2}}\,\delta^{2}
...
\begin{proof}
	忽略噪声对 MFPT 的次主导修正（在顺漂移、小位移下为更高阶），单步\emph{期望}位移
	\(
	\mathbb E[X_{n+1}-X_n\mid X_n=y]
	=
	\mu(y)\Delta/(1+\Delta\lvert\mu(y)\rvert)
	\).
	于是局部\emph{有效速度}
	\(
	v_{\mathrm{tame}}(y)=\lvert\mu(y)\rvert/(1+\Delta\lvert\mu(y)\rvert)
	\),
	跨越 \(\delta\) 的时间可写为
	\(
	...
	\[
	v_{\mathrm{tame}}(y)^{-1}	\frac{1+\Delta\lvert\mu(y)\rvert}{\lvert\mu(y)\rvert}
	=
...
\begin{theorem}[Truncated--EM 的 MFPT 渐近与误差]\label{thm:trunc-mfpt}
...
\begin{corollary}[\(\mu(y)=-y^{3}\) 的显式比较]\label{cor:cubic}
	当 \(\mu(y)=-y^{3}\) 时，
	\begin{align*}
		t^{e}(x,\delta)
		&=\frac{\delta}{x^{3}}+\frac{3}{2}\frac{\delta^{2}}{x^{4}}
...
\begin{remark}
	...
	(2) 若 Truncated--EM 在区间 \([x-\delta,x]\) 内\emph{不}触发截断（即 \(h(\Delta)\gtrsim x\)），则 \(t^{\Delta}\) 与 \(t^{e}\) 具有与 EM 同阶的局部一致性，此时 \eqref{eq:trunc-stepsize} 可放宽。
\end{remark}
...
\subsection{一维立方振子上的时间尺度比较的数值实验}\label{subsec:cubic-numerics}

本小节在具体的一维立方振子模型上，对前文推导的理论时间尺度进行数值检验，系统比较基于生成元离散的空间方法以及基于时间步进的截断 EM 与驯化 EM（tamed EM）
...
\end{equation}
其中漂移项 \(-X^3\) 在大 \(|X|\) 处具有强耗散性，随机扰动由单位强度布朗运动提供。对于给定的大初值 \(x\gg 1\)，我们考察从 \(x\) 向左跨越一个固定距离
\[
L = x - \delta,\qquad 0<\delta\ll x
\]
所需的平均时间。根据前文的漂移主导分析在数值实验中，\(t^e\) 被视为精确时间尺度基准，用于比较不同数值方法的误差。为比较空间离散与时间离散两类方法的性能，采用如下统一设定：\\
初始位置取
	\[
	x\in\{5,\,8,\,12,\,16,\,20\},
	\]
	...
	\]
即在大初值附近考察向左跨越固定距离 \(\delta\) 的时间。对于每个\(x\)，分别用空间离散（SSA）与时间离散（截断 EM、tamed EM）独立模拟 \(N\) 条轨道，估计对应的平均首达时间（MFPT），并与理论值 \(t^e\)做比较。
...
时间离散方法采用固定时间步长 \(\Delta t>0\)。在本节实验中默认 \(\Delta t=10^{-5}\)。为保证对强漂移区域的稳定性与适用性，我们分别使用驯化 Euler（tamed EM）和截断 Euler–Maruyama（truncated EM）两种格式。

\emph{驯化 Euler（tamed EM）} 对应迭代为
...
\emph{截断 Euler–Maruyama（truncated EM）} 首先对状态进行截断，再代入漂移项。设截断半径
\[
...
为比较不同方法在固定空间跨越距离下的时间尺度精度，本文采用以下误差指标。对每个初值 \(x\)，定义相对于理论基准 \(t^e(x\to L)\) 的绝对误差
\begin{align}
	E_u(x) 
...
\begin{figure}[!htbp]
	...
	\caption{一维立方振子模型中，从 \(x\) 向左跨越固定距离 \(\delta\) 的平均时间误差比较：空间离散（改进 \(Q_u \)–SSA）与时间离散（tamed EM、截断 EM）的数值结果。}
	\label{fig:time-space-mfpt}
\end{figure}

从图~\ref{fig:time-space-mfpt}(a) 可以看到，在本节选取的参数范围内，随着初值 \(x\) 增大，三种方法得到的平均首达时间均快速趋近理论基准 \(t^e(x\to L)\)，这与前文的分析相符。其中，基于 \(Q_{u}\) 的 SSA 方法在大 \(x\) 区域的误差明显更小，数值上呈现出随 \(x\) 增大而迅速衰减的趋势，与 \(t^e\) 的主阶行为 \(\delta/x^3\) 一致，体现了空间离散在强漂移极限下对局部时间尺度的良好刻度匹配。图~\ref{fig:time-space-mfpt}(b) 的误差比值进一步揭示：在较大的 \(x\) 下，时间离散方法的误差总体上显著大于空间离散误差，即
\[
R_{\mathrm{tame}}(x),\ R_{\mathrm{trunc}}(x) \gg 1,
\]
表明在同样的空间跨越距离和相近的计算代价下，基于生成元离散的 SSA 方法在该模型上能提供更稳定、更精确的时间尺度估计。随着 \(x\) 增大，空间离散误差已降到非常小的量级，而时间离散误差逐渐受限于时间步长与蒙特卡罗方差，难以进一步降低，这也反映了在强漂移、长时间尺度问题中，单纯减小时间步长未必比提高空间离散精度更划算。

综上，一维立方振子数值实验验证了前文关于在顺漂移和大初值情形下空间离散方法在固定空间跨越距离的时间尺度估计方面具有明显优势的理论结论。

	\section{随机 Canard 慢–快系统的动力学行为与 committor分析}
	...
	\begin{equation}\label{eq:committor_PDE}
		...
		\end{cases}
	\end{equation}
	因此，committor 可以刻画从 $A$ 到 $B$ 的反应轨道的空间分布、流量以及转移速率等统计性质\cite{EV_TPT,EV_TowardsTP,Metzner_TPT_MJP}。
	...
	\begin{equation}\label{eq:canard-SDE}
		...
		\end{cases}
	\end{equation}
	其中$0<\varepsilon \ll 1$为表征慢速变量演化尺度的小参数，$x_t$为快变量，$y_t$为慢变量，$a$是系统参数（当$a$取特定值时出现Canard过渡）。
	
	\subsection{CTRW生成元下的离散committor方程}
	...
	令 \(A_h,B_h\subset\{z_i\}\) 分别为集合 \(A,B\) 在网格上的近似（例如取其内近邻或外近邻）。定义空间离散下的committor为
	\[
	q_h(z_i) := \mathbb{P}_{z_i}\big(\tau_B^h < \tau_A^h\big),
	\]
	其中 \(\tau_A^h,\tau_B^h\) 分别为CTRW过程 \(Z_t^h\) 到达 \(A_h,B_h\) 的首次到达时间。
	
	并且根据\cite{bou2018continuous}对任意内部网格点 \(z_i\in D_h^\circ\setminus(A_h\cup B_h)\)，离散committor满足
	...
	\begin{assumption}\label{ass:L-Qh}
		假设如下条件成立：
		\begin{enumerate}
			\item 系统\eqref{eq:canard-SDE}的漂移与扩散系数在 \(D\) 内 \(C^{4}\) 光滑，且扩散矩阵在 \(D\setminus(A\cup B)\) 上一致椭圆（或满足适当的H\"ormander条件），从而\eqref{eq:committor-PDE}存在唯一解 \(q\in C^{2+\alpha}(D\setminus(A\cup B))\cap C(\overline{D})\)。
			...
			\begin{equation}\label{eq:Qh-consistency}
				...
				\le C h^p.
			\end{equation}
			对于Zu提出的改进 \(\widetilde{Q}_u\) 格式，可取 \(p=2\)。
			
			\item \(Q_h\) 满足离散最大值原理：若 \(u:\{z_i\}\to\mathbb{R}\) 在内部点上满足 \((Q_h u)(z_i)\ge0\)，且在边界上 \(u(z_i)\le 0\)，则对所有网格点有 \(u(z_i)\le0\)。
		\end{enumerate}
	\end{assumption}
	
	在以上假设下，可以给出离散committor的误差界如下：
	
	\begin{theorem}[空间离散下committor的收敛,\cite{bou2018continuous}]\label{thm:Qh-committor-convergence}
		...
		\begin{equation}\label{eq:Qh-error}
			...
			\le C h^p,
		\end{equation}
		其中 \(p\) 由\eqref{eq:Qh-consistency}中的逼近阶决定。对改进的 \(\widetilde{Q}_u\) 格式有 \(p=2\)，因此空间离散下committor函数达到二阶精度。
	\end{theorem}
	...
	\subsection{tamed / truncated EM格式下的committor收敛性}
	
	\subsubsection{tamed / truncated Euler格式与强收敛}
	...
	tamed Euler与truncated Euler–Maruyama格式通过对漂移或状态进行适度削弱或截断，在Khasminskii型条件下恢复全局矩有界性与强收敛性。下述表述为二者的统一结论。
	...
	在假设\ref{ass:Khasminskii}下，可以证明：对任意 \(T>0\) 和 \(q\in[2,p)\)，存在常数 \(C>0\) 与 \(\gamma>0\)，使得tamed Euler或truncated EM近似 \(Z_t^\Delta\) 满足
	...
	\subsubsection{时间离散下的committor分析}
	
	令时间步长 \(\Delta>0\)，离散时刻 \(t_k:=k\Delta\)。tamed / truncated Euler格式给出离散链
	\[
	...
	\]
	表明时间离散下committor的收敛阶不会优于tamed / truncated Euler的强收敛阶。%这与committor作为长时间弱指标的直觉是一致的。
	
	\subsection{随机Canard系统中的比较与讨论}
	
	对随机Canard系统而言，漂移 \(b\) 具有多项式增长，扩散 \(\Sigma\) 有界且光滑，满足Khasminskii型条件\eqref{eq:Khasminskii}，因此tamed Euler与truncated EM格式在有限时间区间内对真实轨道具有强收敛性质\eqref{eq:strong-convergence}。另一方面，在一个包含两个慢流形吸引区 \(A,B\) 的有界区域 \(D\) 内，生成元 \(\mathcal{L}\) 的系数光滑且扩散非退化，committor PDE\eqref{eq:committor_PDE}存在唯一光滑解 \(q\)。
	
	在这样的背景下,空间离散在假设\ref{ass:L-Qh}下，\(q_h\) 以 \(O(h^p)\) 的速率一致收敛到 \(q\)；时间离散通过时间步进构造路径近似 \(Z_t^\Delta\)，再定义离散首次到达时间\eqref{eq:disc-tau}和committor \(q_\Delta\)。利用强收敛\eqref{eq:strong-convergence}与首次到达时间尾部估计\eqref{eq:tail}，命题\eqref{eq:qDelta-to-q}说明 \(q_\Delta\to q\)，但误差阶被强收敛阶所限制，一般不优于 \(O(\Delta^{1/2})\)。要获得可见精度，需要较小时间步长和大量样本路径，因此在计算成本上相对昂贵。
	
	对Canard类快慢系统而言，committor函数度量的是从某一区域出发先落入哪一个慢流形吸引区的概率，这是高度敏感的长时间弱量。从理论收敛阶和结构保持的角度看，基于生成元离散的CTRW空间离散方法，在同等计算代价下更容易获得高精度的committor近似。
	

\subsection{数值实验}\label{sec:canard_cmp}

本节在随机 Canard 慢–快系统~\eqref{eq:canard-SDE} 上开展数值实验，旨在从committor比较基于生成元的空间离散方法（CTRW/\(Q_u\)）与时间离散方法（tamed-EM 与 truncated EM）的数值表现与代价。通过一系列可重复的数值试验，验证前文关于两类格式收敛性质及适用场景的理论分析。
...
在数值方法方面，选取如下两大类：时间离散方法（Time Discretization, TD）：采用 tamed Euler–Maruyama（简记 tamed-EM）与 truncated EM 两种稳定格式，在统一的时间步长 \(\Delta t\) 下对 SDE 进行路径模拟。通过镜像反射实现边界处理，从给定起点发射多条样本路径，根据首次满足 \(x_t\ge x_{\mathrm{th}}\) 或首次进入集合 \(A,B\) 的时间，统计快跳时间分布、committor 与 MFPT。空间离散方法（Space Discretization, SD）：采用基于生成元的一致离散 CTRW/SSA 框架（记为 \(Q_u\) 格式）。在矩形网格上装配离散生成元 \(Q\)，构造最近邻非负跳率，利用 SSA 模拟跳时与通道；对于 committor / MFPT 等弱量，则直接求解离散生成元方程获得全场解。

对于弱量比较，以离散生成元 \(Q\) 上求解的 \(Q\)-解作为参考（ref），在若干评价点 \(\{z_i\}_{i=1}^{N_p}\) 上，以均方根误差
\[
E_q=\Big(\tfrac1{N_p}\sum_{i=1}^{N_p}\lvert \hat q(z_i)-q_{\mathrm{ref}}(z_i)\rvert^2\Big)^{1/2}
\]
度量各方法求得的 committor。空间离散的代价（work）以自由未知元数 \(\times\) 迭代轮数的总局部更新次数为 proxy；时间离散的 work 则以所有评价点、所有样本路径的总时间步数计数，从而在统一的代价指标下绘制 \emph{work--error} 曲线。
...
图~\ref{fig:canard_geom_jump} 给出了在时间窗 \(T=40\) 内，tamed-EM、truncated EM 与 CTRW/SSA 的代表性相图轨道以及对应的首次越阈时间直方图。可以看到，三种方法均能较好再现贴靠—折叠—快跳的典型几何结构：时间离散方法得到的轨道在相图上呈平滑曲线，而 CTRW/SSA 由于采用事件驱动的跳跃过程，在图像上呈现出明显的阶梯特征。
...
%以当前样本数与随机种子为例，三种方法的样本均值分别约为
%\[
%\text{tamed-EM}\approx 18.3,\qquad
%\text{truncated EM}\approx 20.3,\qquad
%\text{CTRW/SSA}\approx 21.4.
%\]
%总体而言，在是否发生快跳以及大致何时发生这一弱指标上，三者给出的结论较为一致；CTRW/SSA 的均值略偏后，主要来源于有限网格分辨率下生成元离散在折叠邻域的误差，此偏差可通过局部加密及改进核 \(\widetilde Q_u\) 进一步减小。

图~\ref{fig:canard_occ} 展示了 tamed-EM 与 CTRW/SSA 在同一时间窗内的长期驻留直方图（occupancy heatmap）。可以看出，两者在整体形状上高度一致，概率质量主要集中在左侧慢流形附近；CTRW/SSA 的占据度在慢流形通道内相对更为集中，体现了事件驱动框架下等待时间显式抽样所带来的驻留拉长效应。
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.60\textwidth]{fig/Occupancy_100.png}
	\caption{占据度热图（长期驻留分布）：左图为 tamed-EM，右图为 CTRW/SSA。可以看到，两者在慢流形通道内的概率质量分布总体一致。}
	\label{fig:canard_occ}
\end{figure}

综合上述结果，在有限时间、以路径几何和快跳时间为指标比较下，时间离散方法（尤其是 tamed/truncated EM）在给定计算代价下更容易获得平滑轨道和较小的路径级误差；CTRW/SSA 在几何结构上也能保持 Canard 的关键特征，但由于跳跃路径的阶梯性，其在路径级强误差指标下并非优势所在。

\subsubsection{Committor 与 MFPT 的空间结构}

为考察 Canard 系统在弱/长期统计量上的差异，本文进一步计算 committor 函数 \(q\) 与平均首达时间（MFPT）\(m\)。它们满足如下生成元型边值问题：
...
图~\ref{fig:committor_mfpt_heat} 给出了 committor 与 MFPT 的热图：左列为 CTRW/\(Q\)-解，右列为 truncated EM Monte Carlo 结果。可以观察到，committor \(q\) 在左侧慢流形区域接近 0，在右侧目标区域 \(B\) 附近接近 1，在折叠邻域形成一条清晰的概率边界层；MFPT \(m\) 在远离 \(B\) 的左支区域取值较大，在靠近 \(B\) 的前沿迅速衰减至 0，体现了几何结构对平均过渡时间的强烈影响。

\begin{figure}[htbp]
	\centering
	\begin{minipage}[b]{0.45\textwidth}
		\includegraphics[width=\textwidth]{fig/committor_heatmap.jpg}
		\caption*{(a) committor, CTRW/\(Q\)-解}
	\end{minipage}\hfill
	\begin{minipage}[b]{0.45\textwidth}
		\includegraphics[width=\textwidth]{fig/committor_trunc_heatmap.png}
		\caption*{(b) committor, truncated EM MC}
	\end{minipage}
	...
	\caption{随机 Canard 系统中 committor 与 MFPT 的空间结构：左列为空间离散（CTRW/\(Q\)）结果，右列为时间离散（truncated EM Monte Carlo）结果。}
...
\subsubsection{基于 committor 的 \texorpdfstring{work--error}{work--error} 比较}
\label{sec:sec63-committor-work-error}

针对 committor 这一生成元型弱量，本文进一步比较空间离散与时间离散在误差—代价关系上的表现。仍以 \eqref{eq:committor-mfpt} 中的 \(q\) 为对象，选择慢流形 \(y=x^3/3-x\) 上的 \(N_p=10\) 个评价点 \(\{z_i\}\)，以较细网格 \(h_{\text{ref}}=0.08\) 下的 \(Q\)-解 \(q_{\text{ref}}\) 作为参考，RMS 误差定义如前所述。实验流程如下：\\
第一步：对于空间离散（SD），在不同网格步长 \(h\in\{0.14,0.12,0.10\}\) 下装配 \(Q\)，采用 Gauss–Seidel 迭代求解 \(Qq=0\)，记录 sweeps 次数与误差 \(E_q\)，并以自由未知元数 \(\times\) sweeps作为 work proxy。\\
第二步：对于时间离散（TD），采用 truncated EM，在统一步长 \(\Delta t=0.004\)、时间窗 \(T_{\max}\approx 18\) 下，从每个评价点发射 \(N\in\{12,24,48,72\}\) 条轨道，通过 Monte Carlo 估计 committor，并以所有节点与路径的总时间步数作为 work。
对应的 \emph{work--error} 曲线见图~\ref{fig:work-error-committor}。可以看到，随着 work 的增加，两类方法的误差均呈下降趋势；但在同一数量级的代价下，空间离散的误差明显更小。

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.62\textwidth]{fig/work_error_committor_trunc_vs_ctrw.pdf}
	\caption{committor 的 \emph{work--error} 曲线：空间离散（CTRW/\(Q\)）与时间离散（truncated EM Monte Carlo）对比。纵轴为相对于参考 \(Q\)-解的 RMS 误差，横轴为代价（work proxy）。}
...
\begin{table}[htbp]
	\centering
	\caption{SD（\(Q\)-解，GS 迭代）在不同网格下的 \emph{work--error}（committor）}
...
这些结果表明，在 committor 与 MFPT 等生成元型弱量的比较中，CTRW/\(Q_u\) 空间离散方法一方面能够自然保持反射边界与几何结构，另一方面通过一次稀疏线性求解即可获得全场解，避免了 Monte Carlo 方差；在同等甚至远低于时间离散的代价下即可达到显著更小的误差，这一结论与前文的理论分析相互印证。
...
本文围绕时间离散与空间离散两类数值格式在随机微分方程模拟中的比较这一主线，选取一维立方振子和随机 Canard 慢–快系统作为代表模型，从理论分析与数值实验两个层面，对改进的显式时间步进方法（tamed Euler、truncated EM）和基于生成元的空间离散方法（CTRW/\(Q_u\) 及其改进格式）进行了系统研究。主要结论可概括如下。

首先，在一维加性噪声 SDE 的漂移主导情形下，本文从固定空间跨越距离所需时间的视角出发，引入了以 ODE 漂移主导时间 \(t^e\) 为参照的时间尺度比较框架。通过对一维立方振子模型的局部渐近分析，证明了在 \(x\gg 1\)、\(\delta\ll x\) 的极限下，改进的 \(Q_u\) 生成元所对应的平均驻留时间 \(t^u\) 与精确漂移时间 \(t^e\) 在主阶与次阶上逐项一致，其相对误差随 \(|x|\to\infty\) 收敛至零，定量说明了空间离散在强漂移区间对局部时间尺度的高精度刻度。相比之下，时间离散方法虽然在强收敛意义下能够逼近真实轨道，但其在固定空间跨越时间上的误差阶受限于时间步进的强收敛阶，通常不优于 \(\mathrm{O}(\Delta^{1/2})\)，要获得与空间离散相当的精度需要更小的时间步长和更高的计算代价。

其次，针对随机 Canard 慢–快系统，本文系统引入了 committor 函数 \(q\) 与平均首达时间（MFPT）\(m\) 这两类生成元型弱量，并将其作为比较时间离散与空间离散优劣的核心指标。在理论方面，一方面，基于随机分析与椭圆型偏微分方程理论，证明了在适当正则性与非退化条件下，CTRW 生成元 \(Q_h\) 所给出的离散 committor 解 \(q_h\) 以 \(\mathrm{O}(h^p)\) 的阶数一致收敛到连续解 \(q\)，对改进的 \(\widetilde Q_u\) 格式可达到二阶空间精度；另一方面，在 Khasminskii 型条件下，分别建立了 tamed Euler 与 truncated EM 对原系统的强收敛估计，并基于路径强收敛和首次到达时间尾部估计，证明了时间离散下的离散 committor \(q_\Delta\) 收敛到 \(q\)，但其误差阶仍受限于时间步进的强收敛阶，只能得到次优的弱收敛控制。这一理论结果从生成元角度揭示了两类方法在 committor 这样的长期弱指标上的本质差异。

在数值实验方面，本文首先在一维立方振子模型上，通过 SSA（基于 \(\widetilde Q_u\) 的 CTRW）与 tamed/truncated EM 的 Monte Carlo 模拟，系统比较了数值平均跨越时间与精确漂移时间 \(t^e\) 的偏差。数值结果表明：在强漂移区域，空间离散所得的平均驻留时间相对于 \(t^e\) 的绝对误差随 \(x\) 增大快速衰减，与理论渐近一致，而时间离散方法的误差在一定步长下则显著受限于时间步进和统计方差。在随机 Canard 系统上，本文分别从路径几何、快跳时间分布、占据度热图以及 committor/MFPT 的空间结构等角度，给出了时间离散方法与 CTRW/\(Q_u\) 的对比。数值结果显示：在有限时间路径级强误差下，tamed-EM 与 truncated EM 更擅长再现 Canard 轨道的几何细节；而在 committor 与 MFPT 等弱/长期指标上，空间离散方法通过一次稀疏线性求解即可获得全场解，且误差–代价曲线明显优于基于 Monte Carlo 的时间离散方法。

进一步的 work–error 曲线定量比较表明：在 committor 的 RMS 误差为例的工作量相同甚至更低的情况下，CTRW/\(Q_u\) 生成元离散往往能够提供远优于 truncated EM Monte Carlo 的数值精度；而在以路径级强误差为度量的有限时间窗内，时间离散方法的误差随 work 单调衰减，并在相同代价下显著优于 CTRW/SSA。这说明在随机 Canard 系统这类典型慢–快模型中，时间离散与空间离散各自适合不同的指标与应用场景：前者适用于追踪单条轨道的几何演化与局部瞬态行为，后者更适用于求解由生成元刻画的弱量与长期统计性质。

综上，本文的主要贡献可概括为三点：其一，提出以固定空间跨越距离所需时间作为时间尺度，构建了时间离散与空间离散两类数值方法的统一比较框架；其二，在一维立方振子与随机 Canard 系统上，分别给出了空间离散与时间离散在漂移主导时间和 committor/MFPT 等弱量上的理论收敛分析；其三，通过一系列数值实验，系统验证了理论分析并给出了误差–代价层面的定量对比，从而对两类方法的适用范围与优势场景给出了较为清晰的判断。
...
总的来看，本文所建立的时间离散与空间离散比较框架，以及在典型模型上的理论与数值分析，为理解随机微分方程数值方法在强精度与弱精度、短时行为与长期统计之间的权衡提供了一个较为清晰的视角。随着数值分析理论与计算资源的不断发展，结合生成元离散、改进时间步进及机器学习等新技术，有望构造出既稳健又高效的随机模拟算法，为更复杂的随机动力系统的定量研究提供有力工具。
