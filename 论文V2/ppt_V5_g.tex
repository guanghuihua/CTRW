...
\begin{frame}{一维立方振子平稳分布的数值验证}
	
	\begin{columns}[c]
		%----------------------------------------------------- 左图：空间离散 SSA
		\column{0.48\textwidth}
		\centering
		% 用 SSA/CTRW 代码画出的“数值平稳分布 vs 精确平稳分布”
		\includegraphics[width=\linewidth]{cubic_ssa_stationary.png}
		\vspace{0.3em}
		
		{\small 空间离散 SSA：$\rho_{\mathrm{SSA}}$ 与精确平稳分布 $\nu$ 的比较}
		
		%----------------------------------------------------- 右图：时间离散 EM
		\column{0.48\textwidth}
		\centering
		% 用 tamed EM + truncated EM 代码画出的“数值平稳分布 vs 精确平稳分布”
		\includegraphics[width=\linewidth]{cubic_em_stationary.png}
		\vspace{0.3em}
		
		{\small 时间离散 tamed / truncated EM：$\rho_{\mathrm{tamed}}$, $\rho_{\mathrm{trunc}}$ 与 $\nu$ 的比较}
	...
	\begin{itemize}
		...
		其精确平稳密度为
		\[
		\nu(x) = Z^{-1} \exp\!\bigl(-\tfrac{1}{4}x^{4}\bigr), 
		\qquad
		Z = \int_{\mathbb{R}} \exp\!\bigl(-\tfrac{1}{4}x^{4}\bigr)\,dx,
		\]
		在代码中用数值积分函数 \texttt{KK()} 进行计算。
		\item 比较方法（空间与时间离散统一到同一空间网格上）：
		\begin{itemize}
			\item 取区间 $[-3,3]$，网格数 $N=120$，网格宽度 $h = \tfrac{6}{120}$，
			网格节点取为 $x_i = -3 + (i+\tfrac12)h$，$i=0,\dots,N-1$。
			\item 空间离散（CTRW/SSA）：在网格上构造马尔可夫链，长时间运行 SSA，
			统计落入每个网格单元的访问频率，得到离散平稳密度近似
			$\rho_{\mathrm{SSA}}(x_i)$。
			\item 时间离散（tamed EM / truncated EM）：在连续空间中用步长 $\Delta t$
			进行时间步进，长时间轨道上对 $X_t$ 做直方图统计（只在同一网格上计数），
			得到 $\rho_{\mathrm{tamed}}(x_i)$、$\rho_{\mathrm{trunc}}(x_i)$。
		\end{itemize}
		\item 在同一网格 $\{x_i\}$ 上，对数值平稳密度与精确平稳密度的差计算离散的
		$L^1$ 误差：
		\[
		\bigl\|\rho_{\mathrm{num}} - \nu\bigr\|_{L^1}
		\approx
		\sum_{i=0}^{N-1} 
		\bigl|\rho_{\mathrm{num}}(x_i) - \nu(x_i)\bigr|\,h.
		\]
		\item 数值结果（把自己代码算出的数值填入下面的占位符）：
		\[
		\bigl\|\rho_{\mathrm{SSA}}   - \nu\bigr\|_{L^1} 
		= \text{(SSA 的误差)},\quad
		\bigl\|\rho_{\mathrm{tamed}} - \nu\bigr\|_{L^1} 
		= \text{(tamed EM 的误差)},\quad
		\bigl\|\rho_{\mathrm{trunc}} - \nu\bigr\|_{L^1} 
		= \text{(truncated EM 的误差)}.
		\]
	\end{itemize}
