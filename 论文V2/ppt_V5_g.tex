...
%			\item 与精确平稳密度在同一网格上比较，
			\item 离散 \(L^{1}\) 误差为
			\[
			\|\rho_{\mathrm{tamed}}-\nu\|_{L^{1}}
			\approx 2.422\times 10^{-2},\quad
			\|\rho_{\mathrm{trunc}}-\nu\|_{L^{1}}
...
\begin{frame}{一维立方振子平稳分布的比较}
	...
	\begin{columns}[T,totalwidth=\textwidth]
		...
		\vspace{0.3em}
		\small
			区间 \([-3,3]\) 上均匀网格，\(N=120\)，\(h=0.05\)
			
			在网格上构造连续时间马尔可夫链，使用 SSA 产生长时间轨道。
			
			统计轨道落入每个网格单元的访问次数，并按总访问次数与 \(h\) 归一化，得到离散平稳密度近似 \(\rho_{\mathrm{SSA}}(x_i)\)。在同一网格上与精确平稳密度比较，离散 \(L^{1}\) 误差
			\(\|\rho_{\mathrm{SSA}}-\nu\|_{L^{1}}
			\approx 6.206\times 10^{-3}\)。
		...
		\small
			...
			使用相同的空间网格 \([-3,3]\)，\(N=120\)，\(h=0.05\)，
			仅在时间方向离散，时间步长 \(\Delta t = 0.001\)。
			
			 对两种时间离散轨道在相同网格上做直方图统计并归一化，
			得到 \(\rho_{\mathrm{tamed}}(x_i)\)、\(\rho_{\mathrm{trunc}}(x_i)\)。
			与精确平稳密度在同一网格上比较，离散 \(L^{1}\) 误差为
			\[
			\|\rho_{\mathrm{tamed}}-\nu\|_{L^{1}}
			\approx 2.422\times 10^{-2},\quad
			\|\rho_{\mathrm{trunc}}-\nu\|_{L^{1}}
