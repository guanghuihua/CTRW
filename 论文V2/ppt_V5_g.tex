...
\begin{document}
	...
	\begin{frame}{研究背景与问题动机}
		\begin{itemize}
			...
			\item 	\([\text{Maruyama, 1955, Rend. Circ. Mat. Palermo.}]\) Euler--Maruyama 方法在全局 Lipschitz 情形下具有 $1/2$ 阶强收敛和 1 阶弱收敛
		
			
			\item \([\text{Milstein, 1975, Theory Probab. Appl.}]\)结合高阶方法可以提高收敛阶，例如Milstein 方法是 1.0 阶强收敛和 1.0 阶弱收敛的
		
			\item \([\text{Hutzenthaler--Jentzen--Kloeden, 2011, Proc. R. Soc. A}].\) 实际模型中漂移 $\mu$、扩散 $\sigma$ 往往只满足局部 Lipschitz 条件，甚至具有超线性增长，
			经典 Euler--Maruyama 格式可能发散
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{两类典型数值方法：时间离散与空间离散}
		\begin{itemize}
			\item 	\( [\text{Bou-Rabee--Vanden-Eijnden, 2018, Mem. Amer. Math. Soc.}] \) 基于生成元的连续时间随机游走（CTRW）方法，由 Bou-Rabee 和 Vanden-Eijnden 系统发展. 在空间网格上离散生成元 $L$ 得到 $Q$ 矩阵，再用 SSA 等算法模拟跳跃过程.
			
			\item 改进的显式时间离散方法：
			\begin{itemize}
				\item 	\( [\text{Hutzenthaler--Jentzen--Kloeden, 2012, Ann. Appl. Probab.}] \)
				
				驯服 Euler--Maruyama（tamed EM）方法
			
				\item 	\( [\text{Mao, 2015, J. Comput. Appl. Math.}] \)
				
				截断 Euler--Maruyama（truncated EM）方法
	...
	\begin{frame}{本文研究内容与主要贡献}
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item 比较时间离散与空间离散在 逃逸概率、MFPT 以及 work--error 指标下的表现.
			\end{itemize}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{时间离散：驯服/截断 Euler--Maruyama}
		\begin{itemize}
			\item 	\( [\text{Hutzenthaler--Jentzen--Kloeden, 2011, Proc. R. Soc. A}]. \)
			对非全局 Lipschitz 系数，经典 EM 可能发散
...
% ===============================================
	...
	\begin{frame}{空间离散：连续时间随机游走（CTRW）}
		\begin{itemize}
			...
			\item 这是 Bou-Rabee 和 Vanden-Eijnden 提出的生成元离散方法的典型形式
		\end{itemize}
	\end{frame}
...
%		\begin{itemize}
%			\item \( [\text{E--Li--Vanden-Eijnden, 2019, Applied Stochastic Analysis, AMS}]. \) It\^o 扩散的无穷小生成元 $L$ 将 SDE 与偏微分方程统一到同一框架
%
%			
%			CTRW/$Q$ 过程通过离散生成元构造有限状态的马尔可夫跳跃过程
%
%			
%			\item \( [\text{Hutzenthaler--Jentzen--Kloeden, 2012, Ann. Appl. Probab.;} \)
...
%			
%			驯服/截断 EM 在局部 Lipschitz 和 Khasminskii 条件下仍具有良好的强收敛性质
%			
...
%	\end{frame}
	...
	\begin{frame}{MFPT 与连续模型的 PDE 表述}
		\begin{itemize}
			\item 	\( [\text{Weinan E等, 2021, Applied Stochastic Analysis, Amer. Math.
			}] .\)
			
			对一维 SDE，以区间端点为吸收边界，平均首达时间 $m(x)$ 满足
			\[
	...
	\begin{frame}{逃逸概率的 work--error 曲线}
		...
		\end{center}
		\vspace{1ex}
		截断 EM 方法与 CTRW 在逃逸概率 RMS 误差下的 work--error 比较.
		\begin{itemize}
