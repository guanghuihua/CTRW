...
def simulate_stationary_histogram(
    ...
    for step in range(total_steps):
        # Step 1: expected increment from rates at current state.
        mu_x, mu_v = underdamped_drift(x, v, gamma)
        qx_p, qx_m, qv_p, qv_m = _rates(mu_x, mu_v, h, sigma, scheme_id)
        exp_dx = tau * h * (qx_p - qx_m)
        exp_dv = tau * h * (qv_p - qv_m)

        # Step 2: midpoint estimate x_hat = x + [omega_bar/2].
        x_hat = _project_to_grid(x + 0.5 * exp_dx, lowx, h)
        v_hat = _project_to_grid(v + 0.5 * exp_dv, lowv, h)
        x_hat = min(max(x_hat, lowx + h / 2.0), lowx + span - h / 2.0)
        v_hat = min(max(v_hat, lowv + h / 2.0), lowv + span - h / 2.0)

        # Step 3: sample Poisson jumps using midpoint rates.
        mu_x_hat, mu_v_hat = underdamped_drift(x_hat, v_hat, gamma)
        qx_p_hat, qx_m_hat, qv_p_hat, qv_m_hat = _rates(mu_x_hat, mu_v_hat, h, sigma, scheme_id)
        dx = np.random.poisson(qx_p_hat * tau) - np.random.poisson(qx_m_hat * tau)
        dv = np.random.poisson(qv_p_hat * tau) - np.random.poisson(qv_m_hat * tau)
...
def plot_accuracy(h_vals: np.ndarray, e_qu: np.ndarray, e_qc: np.ndarray, e_qtu: np.ndarray, out: Path) -> None:
    ...
    ax.set_title("Underdamped Langevin (Midpoint Tau-Leaping): Stationary Density Accuracy")
...
def main() -> None:
    ...
    print("Underdamped Langevin test with midpoint tau-leaping pipeline")
    ...
    out = Path(__file__).resolve().parent / "mid_point_stationary_density_accuracy.png"
