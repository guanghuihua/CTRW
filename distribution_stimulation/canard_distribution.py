...
def ssa_2d(
    ...
    while t < t_stop:
        ...
        # Reversed cumulative selection without allocations
        r = r2 * lam
        if r <= q1:
            mu_number = 4
        elif r <= q1 + q2:
...
def main() -> None:
    ...
    sample = 10E9
    out_n = 100  # target grid for statistics (e.g., 100x100)
    if n % out_n != 0:
    ...
    data2, n_events = ssa_2d(lowx, lowy, span, n, eps, sample, bin_factor)
    ...
    plt.title("SSA 2D Density Surface Plot")
    plt.show()  
