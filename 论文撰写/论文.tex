...
%	%===============================
	...
	\end{cases}
	\]
	在本文中我们统一称 $q$ 为\textbf{先达概率函数（committor）}，并将
	$q(z)\in[0,1]$ 的等值曲线称为\textbf{先达概率等值线}（isocommittor）。
...
时间离散采用 tamed Euler–Maruyama（下称 tamed-EM）与 truncated EM两种稳定格式；空间离散（Space）采用基于生成元的一致离散 \textbf{CTRW/SSA} 框架（下称 $Q_u$），构造最近邻跳率并用 SSA 模拟跳时与通道。
...
\textbf{空间离散（SD）}：CTRW/\(Q_u\) 生成元法，在规则网格上装配 \(Q\)，用高斯--赛德尔迭代（GS）直接求解线性方程；  
\textbf{时间离散（TD）}：\emph{truncated EM} 蒙特卡罗（MC）估计 \(\hat q\)。  
在慢流形 \(y=x^3/3-x\) 上选取 \(10\) 个评价点 \(\{z_i\}\)，以 SD 在较细网格 \(h_{\text{ref}}=0.08\) 的 \(Q\)-解作为\textbf{参考}，并在这些评价点上计算
\[
E_q=\Big(\tfrac1{N_p}\sum_{i=1}^{N_p}\lvert \hat q(z_i)-q_{\text{ref}}(z_i)\rvert^2\Big)^{1/2}.
\]
\textbf{代价（work）}度量：SD 取“自由未知元数 \(\times\) 迭代轮数”（一次局部更新记 1 单位）；TD 取“所有起点与样本路径的\textbf{总时间步数}”。TD 的步长取 \(\Delta t=0.004\)，时间窗 \(T_{\max}\approx 18\)。
...
\begin{table}[htbp]
	...
	\begin{tabular}{cccc}
		...
		\midrule
		172{,}849 & 0.24704 & 27{,}183 & \textbf{0.07024} \\
		309{,}434 & 0.27759 & 27{,}183 & \textbf{0.07024} \\
		667{,}001 & 0.27981 & 27{,}183 & \textbf{0.07024} \\
		924{,}692 & 0.25288 & 27{,}183 & \textbf{0.07024} \\
		\bottomrule
	\end{tabular}
\end{table}

\paragraph{讨论与结论.}
在 committor 这样\textbf{生成元型弱量}的度量上，空间离散（CTRW/\(Q\)）具有天然优势：一次稀疏求解即可得到\emph{全场}的 \(q(x,y)\)，且无 MC 方差；而时间离散需要大量样本和更长时间窗来降低方差与时间插值误差，达到同等精度时代价通常显著更高。上面的\emph{work--error} 曲线与“同等代价”配对表明：当 work 约为 \(2.7\times 10^4\) 时，SD 已把 \(E_q\) 降到 \(\approx 0.07\)；而 TD 在 \(1.7\times 10^5\) 乃至 \(9.2\times 10^5\) 的代价下仍处于 \(0.25\sim0.28\) 的误差区间。由此可见，在随机 Canard 系统的弱量比较中，\textbf{空间离散更高效、更稳健}。
...
\subsubsection{结论}
 在\textbf{有限时间的路径级强误差}度量下（图~\ref{fig:canard_error_work_T10}），tamed/truncated-EM 收敛快、效率高，更适合追踪 Canard 轨道的\emph{几何细节}（贴靠与快跳时序）。
 
 
 在\textbf{弱/长期量与生成元型边值问题}中（图~\ref{fig:canard_occ}--\ref{fig:mfpt_error_work_timeMC}），空间离散（CTRW/$Q_u$）\emph{一次求解}即可获得\emph{全场}解，稳定且无 MC 方差，误差—代价表现更优，且边界条件处理自然。
	\item 
