...
\renewcommand{\thepage}{\Roman{page}}
	...
	\begin{newdescription}{XXXXX}  %第二个 {} 中填写最长的符号和缩略语，以确定列表标号位置的宽度
		...
		\item[$f(x)\asymp g(x)$]
		（渐近同阶）指存在常数 $c_1,c_2>0$ 与 $R>0$，使得对所有足够大的 $x$（默认 $x\ge R$），有
		\[
		...
		\item[$f(x)\sim g(x)$]
		（渐近等价）指 $\displaystyle \lim_{x\to\infty}\frac{f(x)}{g(x)}=1$（或相应极限）。相比 $f\asymp g$ 更强。
	...
	\end{definition}
	 ...
	 \begin{definition}[无穷小生成元]\label{eq:q-matrix-def}
	 ...
	 \begin{theorem}[$Q$-过程的无穷小生成元] \label{ex:q-process-generator}
	 	考虑状态空间 $S = \{1, 2, \dots, I\}$ 上的 $Q$-过程 $X_t$，其生成元 $Q$ 由 \eqref{eq:q-matrix-def}定义。我们有：
	 	\begin{align*}
	 	...
	 	\end{align*}
	 	推导最后一步用到了 \eqref{eq:q-matrix-def}。因此，生成元 $Q$ 正是 $X_t$ 的无穷小生成元。
	 \end{theorem}
...
%	\paragraph{边界条件.}
	...
	\begin{equation}\label{eq:bc}
		...
		\end{cases}
	\end{equation}
	需要说明的是，在本文后续主要考虑的加性噪声情形（\(\sigma\) 常数）下，反射边界对应\textbf{Neumann} 条件 \(\tau'(\xi)=0\)。若 \(\sigma=\sigma(x)\) 非常数，反射可等价为零概率流边界，对于后向问题仍写作 Neumann 型条件，本文采用 \eqref{eq:bc} 的统一写法。
...
\textbf{时间离散（Time）}：采用 \textbf{tamed Euler–Maruyama}（下称 tamed-EM）与 \textbf{truncated EM} 两种稳定格式；  
\textbf{空间离散（Space）}：采用基于生成元的一致离散 \textbf{CTRW/SSA} 框架（下称 $Q_u$），构造最近邻跳率并用 SSA 模拟跳时与通道。
...
以极细 tamed-EM 为共同参考，绘制 tamed/truncated/CTRW 的\textbf{强误差—代价}曲线，结果见图~\ref{fig:canard_error_work_T10}。可见在\textbf{路径级强误差}下，时间法随 work 单调收敛且显著优于 CTRW，这与二者设计目标一致：时间法针对路径级强精度，CTRW 更偏向弱/长期统计。
...
\subsubsection{Committor 与 MFPT的比较}
在离散生成元 $Q$ 上求解 \eqref{eq:committor-mfpt}，一次稀疏方程即可得到\textbf{全场}解。
图~\ref{fig:committor_mfpt_heat} 给出 Committor 与 MFPT 的热图（反射外边界、$q|_A=0,\,q|_B=1,\ m|_B=0$），
图~\ref{fig:committor_mfpt_slice} 给出沿慢流形 $y=x^3/3-x$ 的切片曲线。可以看到：$q$ 在折叠邻域迅速上升，$m$ 在靠近 $B$ 前沿急剧下降，几何效应清晰稳定地体现在解的空间分布中。
...
%\begin{figure}[htbp]
%	\centering
%	\includegraphics[width=0.48\textwidth]{fig/committor_heatmap.jpg}\hfill
%	\includegraphics[width=0.48\textwidth]{fig/mfpt_heatmap.jpg}
%	\caption{左：Committor $q$（$Qq=0$，$q|_A=0,\,q|_B=1$）；右：MFPT $m$（$Qm=-1$ on $\Omega\setminus B$，$m|_B=0$）。}
...
\subsubsection{时间步进 MC 对照——弱量的误差—代价}
为凸显空间离散在弱/长期量上的优势，我们以 $Q$-解作为“地面真实值”，用 tamed-EM 与 truncated-EM 的 Monte Carlo 分别估计若干网格结点的 Committor 与 MFPT，并与 $Q$-解进行\textbf{误差—代价}比较（work 记为所有结点、所有样本的总步数）。结果见图~\ref{fig:committor_error_work_timeMC}--\ref{fig:mfpt_error_work_timeMC}：
时间法的误差随 work 增加而下降，但要达到与 $Q$-解同等精度通常需要显著更高的代价；而空间离散一次稀疏求解即可获得全场分布且无 MC 方差。

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.60\textwidth]{fig/committor_error_work_time.png}
	\caption{Committor：时间步进 MC 相对 $Q$-解的误差—代价曲线（RMS）。}
	\label{fig:committor_error_work_timeMC}
\end{figure}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.60\textwidth]{fig/mfpt_error_work_time.png}
	\caption{MFPT：时间步进 MC 相对 $Q$-解的误差—代价曲线（RMS）。}
	\label{fig:mfpt_error_work_timeMC}
\end{figure}


\begin{figure}[htbp]
	\centering	\includegraphics[width=0.60\textwidth]{fig/committor_error_work_time.png}
	\caption{Committor error vs work using time-stepping Monte Carlo.
		RMS error is measured against the spatial-generator solution $q$ (from $Qq=0$ with $q|_A=0,q|_B=1$).
		Both tamed and truncated EM improve as work increases, yet require substantial steps to match the $Q$-solution, highlighting the advantage of the spatial discretization for weak quantities.}
	\label{fig:committor_error_work_timeMC}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.60\textwidth]{fig/mfpt_error_work_time.png}
	\caption{MFPT error vs work using time-stepping Monte Carlo.
		RMS error is measured against the spatial-generator solution $m$ (from $Qm=-1$ on $\Omega\setminus B$ with $m|_B=0$).
		Due to large variance and truncation bias, time-stepping requires higher work to approach the $Q$-solution, whereas the spatial method yields $m(x,y)$ via a single sparse solve.}
	\label{fig:mfpt_error_work_timeMC}
\end{figure}
...
\subsubsection{小结与讨论}
\begin{itemize}
	\item 在\textbf{有限时间的路径级强误差}度量下（图~\ref{fig:canard_error_work_T10}），tamed/truncated-EM 收敛快、效率高，更适合追踪 Canard 轨道的\emph{几何细节}（贴靠与快跳时序）。
	\item 在\textbf{弱/长期量与生成元型边值问题}中（图~\ref{fig:canard_occ}--\ref{fig:mfpt_error_work_timeMC}），空间离散（CTRW/$Q_u$）\emph{一次求解}即可获得\emph{全场}解，稳定且无 MC 方差，误差—代价表现更优，且边界条件处理自然。
	\item 公平性与稳健性：时间法在不同步长间使用耦合增量；空间法以真实网格细化控制误差。对阈值 $x_{\mathrm{th}}$、集合 $(A,B)$、网格各向异性与局部加密进行小范围扰动，结论不翻转。
\end{itemize}

\subsubsection{实现备注（复现要点）}
\begin{enumerate}
	\item \textbf{共同参考}：用极细步长 tamed-EM 构造 $Z_{\mathrm{ref}}$ 并做双重细化校验，确保参考偏差低于目标阈值。
	\item \textbf{误差—代价口径}：时间法的 work 取总步数；空间法的 work 取总跳数（或一次稀疏求解的浮点运算量）。
	\item \textbf{网格策略}：可在折叠邻域局部加密，并采用各向异性网格 $h_y\approx c\,\varepsilon h_x$，以减少纯漂移方向的离散误差。
\end{enumerate}


============================================

下面把 committor 与 MFPT 的含义、在随机 Canard 系统中的比较思路，以及我们已完成实验的关键结论，做一个面向论文的精炼总结（含必要公式，便于直接放入正文或方法小节）。
...
设二维 Itô SDE
[
\mathrm{d}Z_t=\mu(Z_t),\mathrm{d}t+\Sigma(Z_t),\mathrm{d}W_t,\qquad
Z_t=(x_t,y_t)\in\Omega\subset\mathbb R^2,
]
生成元为 (L f=\mu\cdot\nabla f+\tfrac12\mathrm{Tr}(\Sigma\Sigma^\top\nabla^2 f))。给定两个集合 (A,B\subset\Omega)（本文取
(A={x\le x_A},B={x\ge x_B})；外边界 (\partial\Omega) 反射），定义：

 Committor（先达概率）
[
\boxed{
	\begin{cases}
		L q=0,& \text{in } \Omega!\setminus!(A\cup B),\
		q|_A=0,\quad q|B=1,\
		\text{（反射）}\ \partial\mathbf{n} q=0,& \text{on } \partial\Omega,
\end{cases}}
]
物理意义：从 (z) 出发，先到达 (B) 而非 (A) 的概率 (q(z)=\mathbb P_z(\tau_B<\tau_A))。

 MFPT（首次到达 (B) 的期望时间）
[
\boxed{
	\begin{cases}
		L m=-1,& \text{in } \Omega!\setminus!B,\
		m|B=0,\
		\text{（反射）}\ \partial\mathbf{n} m=0,& \text{on } \partial\Omega.
\end{cases}}
]
物理意义：(,m(z)=\mathbb E_z[\tau_B],)。

这两类量都是弱/分布型指标，天然适合用空间离散的生成元方法来求解与比较。

---

# 2. 我们的比较流程（Space vs. Time）

问题设定（随机 Canard）：
[
\begin{aligned}
	\mathrm{d}x_t &= \big(y_t-(x_t^3/3-x_t)\big),\mathrm{d}t + \sigma_x,\mathrm{d}W_t^{(x)},\
	\mathrm{d}y_t &= \varepsilon,(a-x_t),\mathrm{d}t + \sigma_y,\mathrm{d}W_t^{(y)}.
\end{aligned}
]
基准参数：(\varepsilon=10^{-2},,a=1.0,;(\sigma_x,\sigma_y)=(0,0.08))；域 (\Omega=[-2.5,2.5]\times[-1,3]) 反射；(x_A=-0.2,\ x_B=1.5)。
...
(ii) Time vs. Space：误差—代价（节点集上的 RMS）

 Committor：tamed 与 truncated 的误差随 work 增加而下降；但要匹配 (Q)-解需要相当多的步数。空间离散则通过一次稀疏求解直接得到全场，无 MC 方差。
 MFPT：时间法方差更大、对截断更敏感，曲线下降更慢；空间法依旧一次得到稳定的全场 (m(x,y))。

> 总体趋势：在弱/长期量（committor、MFPT）上，空间离散更高效、更稳健；时间法可以作为交叉验证，但若追求同等精度，通常需要明显更高的 work。

---

# 4. 实践要点与可复现实验步骤（简表）

1. 设定：(\varepsilon=10^{-2},,a=1.0)；(\Omega=[-2.5,2.5]\times[-1,3])（反射）；
(A={x\le -0.2},\ B={x\ge 1.5})。
2. Space：网格（基本 (h_x=h_y\approx 0.06)），组装 (Q_u)；一次解 (Qq=0)、(Qm=-1)，出 (q,m) 热图与慢流形切片。
3. Time：选节点（慢流形附近若干点），tamed-EM 与 truncated-EM MC 估计 (\hat q,\hat m)；统计 RMS 误差与 work，画误差—代价曲线。
4. 稳健性：

 调整节点集（含折叠邻域）、((x_A,x_B))（间距缩放）；
 空间侧在折叠邻域局部加密/各向异性验证误差下降；
 时间侧加大 (N)、缩小 (\Delta t)、延长上限时间，观察曲线单调下降与置信带收缩。

---

# 5. 一句话结论（写在小结里）

> 对随机 Canard 系统，committor 与 MFPT 等生成元型弱量更适合用空间离散（CTRW/(Q_u)）直接求解：一次稀疏线性方程即可得到全场分布，稳定且无 Monte Carlo 方差；而时间离散（tamed/truncated EM）作为蒙特卡洛估计虽能收敛，但在达到同等精度时通常需要显著更高的计算代价。
