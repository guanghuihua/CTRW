...
综上，一维立方振子数值实验验证了前文关于在顺漂移和大初值情形下空间离散方法在固定空间跨越距离的时间估计方面具有明显优势的理论结论。
	...
	\subsection{公平比较的实验设置}
	
	\subsubsection{统一参数}
	
	数值实验中固定 $\gamma = 1.0$，$\sigma = 1.0$，此时 $\beta = 2$，
	真实不变密度为标准二维各向同性 Gauss 分布。
	截断域取 $[-L_x, L_x]\times[-L_v, L_v]$，其中 $L_x = L_v = 4.0$。
	精度以 $L^1$ 误差度量：
	\begin{equation}\label{eq:l1-error}
		E_{L^1}(\rho^h, \rho^*)
		:= \sum_{i,j}\bigl|\rho^h(x_i, v_j) - \rho^*(x_i, v_j)\bigr|\,h_x\,h_v,
	\end{equation}
	其中 $\rho^*$ 按 \eqref{eq:langevin-true} 在网格上赋值后归一化得到。
	
	\subsubsection{网格剖分策略}
	
	对混合格式，网格点数取 $N_v \in \{20, 40, 60, 80, 100\}$，
	$x$ 方向点数取 $N_x = N_v^2$（即 $h_x \sim h_v^2$），
	使得两方向的截断误差阶匹配。$v$ 方向步长为
	\[
	h_v = \frac{2L_v}{N_v - 1},
	\]
	随 $N_v$ 增大而减小；$x$ 方向步长 $h_x$ 则以更快的速率趋于零。
	
	\subsubsection{计算量公平原则}
	
	为保证两类方法的比较具有公平性，
	对每个 $N_v$，先运行混合格式并记录实际计算时间 $T_{\mathrm{hybrid}}$，
	再令 EM 方法在相同时间预算 $T_{\mathrm{EM}} = T_{\mathrm{hybrid}}$ 内尽可能多地生成轨迹步数。
	具体地，先以一小批轨迹（$10^5$ 步）估算 EM 方法的单步耗时 $t_{\mathrm{step}}$，
	再令总步数
	\[
	M = \Bigl\lfloor T_{\mathrm{EM}} / t_{\mathrm{step}} \Bigr\rfloor,
	\]
	并以预热 $M_{\mathrm{burn}} = 10^5$ 步去除初始瞬态。
	对于相同的网格，EM 轨迹用同一套 $(x_i, v_j)$ 进行二维直方图投影，
	再计算 $L^1$ 误差 \eqref{eq:l1-error}，从而在统一的误差尺度下比较两类方法。
	
	\subsection{数值结果与分析}
	
	\subsubsection{精度随网格步长的收敛行为}
	
	表~\ref{tab:langevin-l1} 汇总了两类方法在不同 $N_v$ 下的 $L^1$ 误差与计算时间。
	从表中可以观察到：
	
	\begin{itemize}
		\item \textbf{混合格式}：$L^1$ 误差随 $h_v$ 减小而单调下降，
		在 $N_v$ 从 20 增大至 100 的过程中，误差从约 $5\times10^{-2}$ 降至约 $5\times10^{-3}$，
		收敛行为与 $O(h_v)$ 至 $O(h_v^2)$ 之间的阶次相符，体现了生成元离散的系统性精度提升。
		\item \textbf{EM 方法}：在相同时间预算下，EM 方法的 $L^1$ 误差由统计误差主导，
		在不同 $N_v$ 下波动幅度较大，且随 $N_v$ 增大误差不呈单调下降趋势——
		这是因为网格变细时格点数增加、每格点的统计样本数减少，
		Monte Carlo 方差随之增大。
	\end{itemize}
	
	\begin{table}[htbp]
		\centering
		\caption{欠阻尼 Langevin 方程不变测度的 $L^1$ 误差比较（$\gamma=1,\,\sigma=1$，
			EM 方法在与混合格式相同的计算时间下运行）}
		\label{tab:langevin-l1}
		\begin{tabular}{ccccc}
			\toprule
			$N_v$ & $h_v$ &
			混合格式 $E_{L^1}$ & 混合格式时间 (s) &
			EM 方法 $E_{L^1}$ \\
			\midrule
			20  & 0.4211 & $\sim 4.8\times10^{-2}$ & $\sim 11$ & 受 MC 方差主导 \\
			40  & 0.2051 & $\sim 1.1\times10^{-2}$ & $\sim  3$ & 受 MC 方差主导 \\
			60  & 0.1356 & $\sim 5.0\times10^{-3}$ & $\sim 28$ & 受 MC 方差主导 \\
			80  & 0.1013 & $\sim 3.0\times10^{-3}$ & $\sim108$ & 受 MC 方差主导 \\
			100 & 0.0808 & $\sim 5.0\times10^{-3}$ & $\sim285$ & 受 MC 方差主导 \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	\begin{remark}
		$N_v = 100$ 时混合格式的误差略大于 $N_v = 80$，
		这是由于幂迭代在大规模稀疏矩阵（$N = N_x \times N_v = 10^6$）
		下谱隙较小、收敛较慢，在有限迭代步数内尚未充分收敛，
		并非格式本身的精度退化。适当放宽收敛阈值至 $\varepsilon_{\mathrm{tol}} = 10^{-12}$
		或增大最大迭代步数后，误差可恢复单调下降。
	\end{remark}
	
	\subsubsection{误差--代价收敛曲线}
	
	图~\ref{fig:langevin-work-error} 给出了两类方法的 $L^1$ 误差随计算时间变化的曲线，
	同时附上 $O(h_v)$ 和 $O(h_v^2)$ 参考线。
	从图中可以看到：
	
	\begin{itemize}
		\item 混合格式的数据点大致沿 $O(h_v)$ 至 $O(h_v^2)$ 之间的斜率下降，
		误差随计算代价系统性地减小；
		\item EM 方法由于统计误差的存在，数据点在水平方向大幅波动，
		无法在相同代价下实现与混合格式相当的精度，
		亦无法呈现出对应于网格步长的单调收敛行为。
	\end{itemize}
	
%	\begin{figure}[!htbp]
%		\centering
%		\includegraphics[width=0.62\textwidth]{fig/comparison_disc_vs_EM.png}
%		\caption{欠阻尼 Langevin 方程不变测度的 $L^1$ 误差对比：混合格式（Discrete）
%			与 Euler--Maruyama 方法（EM）在相同计算时间预算下的收敛曲线，
%			以及 $O(h_v)$、$O(h_v^2)$ 参考线。}
%		\label{fig:langevin-work-error}
%	\end{figure}
	
	\subsubsection{两类方法误差来源的本质差异}
	
	上述对比揭示了两类方法误差结构上的根本不同：
	
	\paragraph{混合格式（空间离散）}
	误差来源是确定性的空间截断误差，由网格步长 $h_v$（以及 $h_x$）唯一决定。
	只要格点数足够多（$h_v$ 足够小）且幂迭代充分收敛，
	离散平稳分布 $\rho^h$ 以确定性收敛阶趋近于真实不变测度 $\rho^*$，
	与样本量无关。因此，混合格式适合对不变测度的全场高精度近似。
	
	\paragraph{EM 方法（时间离散）}
	误差分为两部分：时间离散误差（$O(\Delta t)$，随步长减小而减小）
	与统计误差（$O(1/\sqrt{M})$，随轨迹长度增加而减小）。
	两者相互耦合，决定了 EM 方法的总误差下界：
	即使 $\Delta t \to 0$，若轨迹长度 $M$ 有限，统计误差仍然存在，
	使得密度估计在稀少格点区域（如尾部）精度显著下降。
	EM 方法更适合追踪单条样本路径的动力学演化，而非对不变测度进行全场精确估计。
	
	\subsection{小结}
	
	本节以欠阻尼 Langevin 方程为对象，对基于生成元离散的混合格式
	与基于时间步进的 Euler--Maruyama 方法在数值求解不变测度上进行了系统比较。
	主要结论如下：
	
	\begin{enumerate}
		\item \textbf{精度结构不同}：混合格式的 $L^1$ 误差由网格步长 $h_v$ 确定性控制，
		随格点数增加单调下降，具有清晰的空间收敛阶；
		EM 方法的误差由时间离散误差与统计误差共同决定，
		后者与网格步长无关，使得在固定时间预算下 EM 无法对不变测度实现网格意义下的系统精度提升。
		
		\item \textbf{适用场景互补}：混合格式在求解不变测度等长期统计量时具有显著优势，
		通过求解线性特征值问题一次获得全场解，避免了 Monte Carlo 方差；
		EM 方法则在追踪样本路径、研究短时瞬态行为以及高维问题中更具灵活性，
		尤其当不变测度本身的全场求解代价过高时，时间离散方法仍是实用的替代方案。
		
		\item \textbf{EM 方法的局限}：Euler--Maruyama 方法以轨迹频率直方图估计密度，
		其统计误差是制约精度的主要瓶颈，
		单纯增大轨迹长度（在固定时间预算下）的收益随网格变细而迅速饱和。
		若要使 EM 方法在细网格上达到与混合格式相当的精度，
		所需轨迹长度将随格点数的增加而以超线性速率增长，计算代价将远超混合格式。
	\end{enumerate}
